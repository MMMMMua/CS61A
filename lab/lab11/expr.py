...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        if self.string in env:
            ...
            raise NameError('name {} not found'.format(self.string))
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        Number(14)
        """
        evalOperator = self.operator.eval(env)
        evalOperands = [operand.eval(env) for operand in self.operands]

        return evalOperator.apply(evalOperands)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        parentEnv = self.parent.copy()
        tempEnv = {param : arg for param, arg in zip(self.parameters, arguments)}
        
        for param in tempEnv:
            parentEnv[param] = tempEnv[param]

        return self.body.eval(parentEnv)
